...
class CategoriaProductoViewSet(viewsets.ModelViewSet):
    ...
    permisos_por_accion = {
        "create": "catalogos.gestionar",
        "update": "catalogos.gestionar",
        "partial_update": "catalogos.gestionar",
        "destroy": "catalogos.gestionar",
    }
...
class ProductoViewSet(viewsets.ModelViewSet):
    ...
    def capas(self, request, pk=None):
        """Lotes/capas activas del producto, enriquecidas con fecha de
        vencimiento y documento de compra (HU07: controlar inventario por lotes)."""
        from apps.compras.models import DetalleCompra

        producto = self.get_object()
        capas = list(
            producto.capas_costo.filter(estado="ACTIVA").order_by("fecha_ingreso", "id")
        )

        # Documento de compra (factura/orden) por compra_id, solo para mostrar
        compra_ids = [c.origen_id for c in capas if c.origen == "COMPRA" and c.origen_id]
        documentos = {}
        if compra_ids:
            detalles = DetalleCompra.objects.filter(
                compra_id__in=compra_ids, producto=producto
            ).select_related("compra")
            for det in detalles:
                documentos[det.compra_id] = (
                    det.compra.numero_factura
                    or det.compra.numero_orden
                    or f"Compra {det.compra_id}"
                )

        data = []
        for c in capas:
            documento = documentos.get(c.origen_id, "") if c.origen == "COMPRA" else ""
            data.append(
                {
                    "id": c.id,
